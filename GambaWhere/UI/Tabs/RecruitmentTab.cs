...
/// <summary>
/// Shared engine for the recruitment tabs: browses and posts venue or host listings, parameterised by post type.
/// FindAVenueTab and FindAHostTab are thin wrappers that each hold one of these.
/// </summary>
internal sealed class RecruitmentTab : IDisposable
{
    ...
    private void DrawCardHeader(RecruitmentPost post, float contentWidth, float rightReserve)
    {
        ...
        if (_isVenue)
        {
            var tex = !string.IsNullOrWhiteSpace(post.ImageUrl) ? _imageService.GetFromUrl(post.ImageUrl!) : null;
            if (tex != null)
        ...
        {
            var tex = !string.IsNullOrWhiteSpace(post.ProfileImageUrl) ? _imageService.GetFromUrl(post.ProfileImageUrl!) : null;
            if (tex != null)
    ...
    private void DrawCardButtons(RecruitmentPost post, float rowWidth)
    {
                   ...
                   .Push(ImGuiCol.ButtonActive, ThemeColours.ButtonPressed(_config.PrimaryColour)))
        {
            ...
            if (UIHelper.IconTextButton(FontAwesomeIcon.CommentDots, "Contact", buttonSize, "##rec_contact"))
                CopyTellToClipboard(post.PosterCharacter);
        }
    }

    private void CopyTellToClipboard(string posterCharacter)
    {
    ...
    private static bool TryBuildTellCommand(string posterCharacter, out string command, out string target)
    {
    ...
    private void DrawProfilePopup()
    {
        ...
        ProfilePopup.Draw($"##rec_profile_popup_{IdScope}", ref _openProfileRequested, _imageService, _config, data);
    }
