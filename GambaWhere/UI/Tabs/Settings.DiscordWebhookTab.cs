...
public sealed class DiscordWebhookTab
{
    ...
    private string? TryApplyBanner(string sourcePath, string slotName, Func<string?> getFileName, Action<string?> setFileName)
    {
        ...
        var saved = _imageService.SaveBanner(trimmed, slotName);
        if (saved == null)
        ...
        Action<string?> setFileName)
    {
        ...
        if (hasCustom)
        {
            ...
            {
                _imageService.DeleteBanner(customFileName);
                _imageService.ReloadImages();
                setFileName(null);
                applyError = null;
            }
        ...
        var previewPath = hasCustom
            ? _imageService.GetBannerPath(customFileName)
            : null;

        if (previewPath != null)
        {
            var tex = _imageService.GetFromPath(previewPath);
            if (tex != null && tex.Width > 0 && tex.Height > 0)
            {
                ImGuiHelpers.ScaledDummy(4f);
    ...
    private void KickApply(DiscordWebhookEntry entry)
    {
        _ = Task.Run(async () =>
        {
            ...
            {
                _log.Warning(ex, "Discord webhook delivery failed unexpectedly.");
            }
        });
    }
